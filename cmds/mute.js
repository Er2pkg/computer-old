...
const row = Comp.DB.mutes.find(i => i.guild == message.guild.id && i.id == message.author.id)
if(!row) {Comp.DB.mutes.set(message.guild.id+'_'+message.author.id, new Comp.structures.Mute('', {guild: message.guild.id, id: message.author.id, inmute: 1, reason: (message.args.slice(2).join(' ') || 'no reason'), mute_time: Date.now(), unmute_time: (Date.now()+parseInt(message.args[1])) }))
