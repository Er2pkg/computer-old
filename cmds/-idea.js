const methods = ['лист', 'list', 'добавить', 'add', 'просмотр', 'show', 'просмотреть', 'удалить', 'remove', 'delete', 'изменить', 'edit', 'оценить', 'rate'], ideaHelp = {
...
if(!message.args[0] || !methods.includes(message.args[0].toLowerCase())) {message.channel.stopTyping(); return message.channel.send(ideaHelp[message.lang]);}
...
Comp.con.query(`INSERT INTO ideas (user, name, date, message) VALUES ('${message.author.id}', '${message.args.slice(1).join(' ').replace(/\'/g, '\\\'').slice(0,10)}', ${Date.now()} '${message.args.slice(1).join(' ').replace(/\'/g, '\\\'')}')`)
if(rows.length < 1) message.channel.send(ph[3]+'1')
else message.channel.send(ph[3]+(rows.length+1))
break
case methods[4]:
case methods[5]:
case methods[6]: //show
...
find = Comp.con.query(`SELECT * FROM ideas WHERE id=${parseInt(message.args[1])}`)[0]
if(!find) return message.reply(ph[5])
else message.reply('\n'+find.text)
break
case methods[7]:
case methods[8]:
case methods[9]: //delete
message.channel.stopTyping()
...
Comp.con.query(`DELETE FROM ideas WHERE user='${message.author.id}' ${find!=='all'?'AND id='+find.id:''}`)
message.channel.send(find=='all'?ph[7]:ph[8]+find.id)}
break
case methods[10]:
case methods[11]: //edit
message.channel.stopTyping()
if(!message.args[1]) return message.reply(ph[2])
if(isNaN(parseInt(message.args[1]))) return message.reply(ph[4])
find = rows.find(i => i.id == parseInt(message.args[1]))
...
Comp.con.query(`UPDATE ideas SET name='${message.args.slice(2).join(' ').replace(/\'/g, '\\\'').slice(0,10)}', text='${message.args.slice(2).join(' ').replace(/\'/g, '\\\'')}' WHERE user='${message.author.id}' AND id=${find.id}`)
message.channel.send(ph[10]+find.id+ph[11])
break
case methods[12]:
case methods[13]: //rate
