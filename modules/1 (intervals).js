...
Comp.client.users.cache.forEach(u => u.bot?'':Comp.DB.preds.get(u.id)?'':Comp.DB.preds.set(u.id, new Comp.structures.Pred('', {id: u.id})))
Comp.client.users.cache.forEach(u => u.bot?'':Comp.DB.ignores.get(u.id)?'':Comp.DB.ignores.set(u.id, new Comp.structures.Ignore('', {id: u.id})))
}, 5000)
