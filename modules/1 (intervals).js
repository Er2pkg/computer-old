...
.addField('Процессор', Math.avg(Comp.client.stats.cpu), true)
...
Comp.client.users.cache.forEach(u => u.bot?'':Comp.DB.preds.get(u.id)?'':Comp.DB.preds.set(u.id, new Comp.classes.Pred({id: u.id})))
Comp.client.users.cache.forEach(u => u.bot?'':Comp.DB.ignores.get(u.id)?'':Comp.DB.ignores.set(u.id, new Comp.classes.Ignore({id: u.id})))
}, 5000)
...
if(newDB.length > 0 && row && row._deleted) {Comp.DB[t].delete(key); Comp.db.query(`DELETE FROM ${t} WHERE id = ${i.id+(i.guild?' AND guild = '+i.guild:'')}`)}
else if(unmatches.length > 0 && newDB.length > 0 && !row._deleted) Comp.db.query(`UPDATE ${t} SET ${unmatches.map(i => i.key+' = \''+newDB[i.index]+'\'').join(', ')} WHERE id = '${i.id+(i.guild?('\' AND guild = \''+i.guild):'')}'`)
